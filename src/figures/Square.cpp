...
Square::Square(const vec2& center, const int width, const int height)
    : m_center(center), m_width(width), m_height(height), m_vbo(initData(), 4 * 2 * sizeof(float)), m_ibo(initIndices(), 6)
{}
...
float* Square::initData() {
    ...
    float data[] = {
        ...
        m_vertices[1].x, m_vertices[1].y, // 1
        m_vertices[2].x, m_vertices[2].y, // 2
        m_vertices[3].x, m_vertices[3].y  // 3
    };
